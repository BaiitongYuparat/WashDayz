...
import { getOrderById, OrderDetail, putOrderStatus } from "@/services/orderService"
...
export default function OrderTrackingScreen() {
  ...
  const handleCancel = () => {
    Alert.alert(
      "ยกเลิกคำสั่งซื้อ",
      "คุณต้องการยกเลิกคำสั่งซื้อนี้ใช่ไหม?",
      [
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        ...
        <View className="bg-blue-main rounded-3xl p-5 mb-4">
          <View className="flex-row items-center justify-between mb-3">
            ...
            <Text className="text-white/70 text-xs">#{order.order_id.slice(0, 8)}</Text>
          </View>
          ...
          <View className="mt-4 gap-1">
            {order.items.map((item) => (
              <View key={item.order_item_id} className="flex-row items-center gap-2">
                <MaterialCommunityIcons name="washing-machine" size={14} color="rgba(255,255,255,0.7)" />
                <Text className="text-white/70 text-xs">
                  {MACHINE_TYPE_LABELS[item.machine.type] ?? item.machine.type} · {item.machine.capacity} กก.
                </Text>
              </View>
            ))}
          </View>
        ...
        {order.status === "COMPLETED" && (
          <View className="bg-green-50 rounded-3xl p-5 mb-4 border border-green-100 flex-row items-center gap-3">
            ...
            <View className="flex-1">
              <Text className="font-bold text-green-800 text-sm">ผ้าพร้อมรับแล้ว!</Text>
        ...
        {order.status === "CANCELLED" && (
          <View className="bg-red-50 rounded-3xl p-5 mb-4 border border-red-100 flex-row items-center gap-3">
            <MaterialCommunityIcons name="close-circle-outline" size={28} color="#A32D2D" />
            <View className="flex-1">
              <Text className="font-bold text-red-800 text-sm">คำสั่งซื้อถูกยกเลิกแล้ว</Text>
              <Text className="text-red-400 text-xs mt-0.5">หากมีข้อสงสัยกรุณาติดต่อสาขา</Text>
            </View>
          </View>
        )}
